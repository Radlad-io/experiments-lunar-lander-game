...
padMesh.position.set(-100, 15, -50);
...
pad2Mesh.position.set(20, 15, 75);
...
const updateVelocity = () => {
  ...
  horizVelocityText.text = `${
    ...
    cameraSide
      ? Math.abs(landerBodyPhysics.velocity.z.toFixed(1))
      : Math.abs(landerBodyPhysics.velocity.x.toFixed(1))
  }m/s`;
};
...
let Key = {
  ...
  onKeydown: function (event) {
    ...
    if (event.key === "Shift" && !Object.keys(Key._pressed).includes("Shift")) {
      checkRotation();
    }
